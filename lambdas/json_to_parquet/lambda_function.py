...
def send_alert(subject: str, message: str):
    """Safely send failure alert via SNS if configured correctly."""
    if SNS_TOPIC and SNS_TOPIC.startswith("arn:aws:sns:"):
        try:
            ...
            logger.info("Alert notification published to SNS.")
        except Exception as e:
            logger.error(f"Failed to publish notification to SNS: {e}")
    else:
        logger.warning(f"SNS alert skipped. Topic ARN is empty or misconfigured: '{SNS_TOPIC}'")
