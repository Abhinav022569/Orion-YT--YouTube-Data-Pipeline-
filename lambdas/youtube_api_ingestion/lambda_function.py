...
def send_alert(subject: str, message: str):
    """Send failure alert via SNS with structural validation."""
    # Ensure SNS_TOPIC is configured and looks like a valid AWS ARN string
    if SNS_TOPIC and SNS_TOPIC.startswith("arn:aws:sns:"):
        try:
            sns_client.publish(
                TopicArn=SNS_TOPIC,
                Subject=subject[:100],
            ...
            logger.info("Failure alert successfully sent to SNS topic.")
        except Exception as e:
            logger.error(f"Failed to publish alert to SNS ARN '{SNS_TOPIC}': {e}")
    else:
        logger.warning(
            f"SNS alert skipped. 'SNS_ALERT_TOPIC_ARN' is either empty, "
            f"misconfigured, or invalid. Current value: '{SNS_TOPIC}'"
        )
