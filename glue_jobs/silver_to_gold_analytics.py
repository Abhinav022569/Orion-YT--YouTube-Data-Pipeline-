...
─────────────────────────────────────────────────
Upgraded Pipeline:
  - Standardizes mathematical aggregations against 0-division Nulls.
  - Adds schema tracking stability for automated catalog creation.
  - Removes redundant structural counts for faster spark task evaluation.
"""
...
logger.info("Reading Silver reference data for category names...")
try:
    ...
    available_cols = ref_df.columns
    
    if "id" in available_cols and "snippet.title" in available_cols:
        category_lookup = ref_df.select(
            F.col("id").cast("long").alias("category_id"),
            F.col("snippet.title").alias("category_name"),
        ).dropDuplicates(["category_id"])
    elif "id" in available_cols and "snippet_title" in available_cols:
        category_lookup = ref_df.select(
            F.col("id").cast("long").alias("category_id"),
            F.col("snippet_title").alias("category_name"),
        ).dropDuplicates(["category_id"])
    else:
        logger.warn(f"Could not resolve category title columns. Available: {available_cols}")
    ...
    if category_lookup is not None:
        ...
        # Broadcast small reference dataset for performance optimization
...
# Guarantee category_name column exists cleanly and fallback empty records to Unknown
...
# Handle metric protection against divide-by-zero or empty rows
metric_cols = ["views", "likes", "dislikes", "comment_count", "like_ratio", "engagement_rate"]
for col in metric_cols:
    if col in stats_df.columns:
        stats_df = stats_df.withColumn(col, F.coalesce(F.col(col), F.lit(0)))
...
# ══════════════════════════════════════════════════════════════════════════════
# GOLD TABLE 2: Channel Analytics (Fixed Collection Serialization)
# ══════════════════════════════════════════════════════════════════════════════
...
channel = stats_df.groupBy("channel_title", "region").agg(
    ...
    # Fixed: Convert array type set to a flat string column to prevent parquet writer crashes
    F.array_join(F.collect_set("category_name"), ", ").alias("categories"),
)
...
category = category.withColumn(
    "view_share_pct",
    F.round(F.coalesce(F.col("total_views") / F.sum("total_views").over(window_total) * 100, F.lit(0.0)), 2)
)
